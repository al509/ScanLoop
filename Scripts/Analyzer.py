...
class Analyzer(QObject):
            ...
            if isinstance(loaded_object, SNAP_experiment.SNAP):
                ...
                try:
                    ...
                    self.S_print_error.emit(
                        'Spatial scale is defined as steps 2.5 um each')
    ...
    def resave_SNAP(self, output_file_type='SNAP'):
        try:
            ...
            i_x_min = np.argmin(
                abs(self.SNAP.positions[:, self.SNAP.axes_dict[self.SNAP.axis_key]]-x_lim[0]))
            i_x_max = np.argmin(
                abs(self.SNAP.positions[:, self.SNAP.axes_dict[self.SNAP.axis_key]]-x_lim[1]))
        ...
        if output_file_type in ['SNAP', 'cSNAP']:
            ...
            new_SNAP.signal = self.SNAP.signal[i_w_min:i_w_max,
                                               i_x_min:i_x_max]
            ...
            if output_file_type == 'SNAP':
                ...
                NewFileName = path+'\\' + \
                    FileName.split('.')[-2]+'_resaved.cSNAP'
                new_SNAP.jones_matrixes_used = True
                new_SNAP.jones_matrixes_array = self.SNAP.jones_matrixes_array[
                    i_w_min:i_w_max, i_x_min:i_x_max, :, :]
    ...
    def delete_slice(self, position):
        ...
        f.close()
        self.S_print.emit(
            'Index {} deleted. Data resaved to {}'.format(index, NewFileName))
    ...
    def save_single_spectrum(self):
        ...
        if self.slice_position is None:
            ...
            if FileName.endswith('.SNAP'):
                FileName = FileName[:-5]
            if FileName.endswith('.pkl3d'):
                FileName = FileName[:-6]
            NewFileName = path+'\\'+FileName + \
                '_at_{}'.format(self.slice_position)
    ...
    def plot_spectrogram(self):
        ...
        try:
            if self.SNAP.type_of_signal != self.type_of_spectrogram:
                if self.SNAP.jones_matrixes_used:
                    ...
                    self.S_print_error.emit('Error. No complex data in SNAP object to derive {}. Only {} is available'.format(
        ...
        x = self.SNAP.positions[:,
                                self.SNAP.axes_dict[self.SNAP.axis_key]]-p['shift_x_axis']
        if p['shift_x_axis'] != 0:
            self.S_print_error.emit(
                '!!! Note that X axis in spectrogram are shifted on {} um according to plotting_parameters.txt'.format(p['shift_x_axis']))
        ...
        def _convert_ax_Wavelength_to_Radius(ax_Wavelengths):
            ...
            nY2 = (y2-lambda_0)/lambda_0*self.SNAP.R_0 * \
                self.SNAP.refractive_index*1e3
        ...
        if p['ERV_axis']:
            ax_Radius = ax_Wavelengths.secondary_yaxis(
                'right', functions=(_forward, _backward))
        ...
        if p['position_in_steps_axis']:
            ...
            try:
                clb = fig.colorbar(
                    im, ax=ax_steps, pad=p['colorbar_pad'], location=p['colorbar_location'])
            except TypeError:
                self.S_print.emit(
                    'WARNING: update matplotlib up to 3.4.2 to plot colorbars properly')
                clb = fig.colorbar(im, ax=ax_steps, pad=p['colorbar_pad'])
        else:
            try:
                clb = fig.colorbar(
                    im, ax=ax_Wavelengths, pad=p['colorbar_pad'], location=p['colorbar_location'])
            except TypeError as e:
                self.S_print.emit(
                    'WARNING: update matplotlib up to 3.4.2 to plot colorbars properly. Error is ' + str(e))
                clb = fig.colorbar(im, ax=ax_Wavelengths,
                                   pad=p['colorbar_pad'])
    ...
    def plot_sample_shape(self):
        ...
        ax.plot(
            self.SNAP.positions[:, 2], self.SNAP.positions[:, 0], self.SNAP.positions[:, 1])
    ...
    def analyze_spectrum(self, fig):
        ...
        if len(peakind2) > 0:
            ...
            main_peak_index = np.argmax(
                abs(signal[peakind2]-np.nanmean(signal)))
            # wavelength_main_peak=waves[peakind2][main_peak_index]
            # sort in wavelength decreasing
            peakind2 = peakind2[np.argsort(-waves[peakind2])]
            ...
            if plot_with_high_point_number:
                waves_fitted = np.linspace(wave_min, wave_max,10000)
                signal_fitted = SNAP_experiment.Fano_lorenzian(
                    waves_fitted, non_res_transmission, Fano_phase, resonance_position, delta_0, delta_c)
                axes.plot(waves_fitted, signal_fitted, color='green')
            else:
                signal_fitted = SNAP_experiment.Fano_lorenzian(
                    waves, non_res_transmission, Fano_phase, resonance_position, delta_0, delta_c)
    ...
    def apply_FFT_to_spectrum(self, fig):
        ...
        def FFTFilter(y_array):
            ...
            # plt.plot(W,f_array)
            Indexes = [i for i, w in enumerate(W) if any(
                [w < LowFreqEdge, abs(w) > HighFreqEdge])]
            f_array[Indexes] = 0
            # plt.plot(W,f_array)
            # plt.ylim((-1000,1000))
    ...
    def extract_ERV(self):
        ...
        if self.figure_spectrogram is not None and self.indicate_ERV_on_spectrogram:
            if len(self.figure_spectrogram.axes[0].lines) > 1:
                for line in self.figure_spectrogram.axes[0].lines[1:]:
                    line.remove()
            ...
            for i in range(0, self.number_of_peaks_to_search):
                self.figure_spectrogram.axes[0].plot(x, peak_wavelengths[:, i])
                line = self.figure_spectrogram.axes[0].plot(
                    x, peak_wavelengths[:, i])
    ...
    def get_modes_parameters(self):
        ...
        if len(modes_parameters) > 0:
            for D in modes_parameters:
                if self.figure_spectrogram is not None:
                    self.figure_spectrogram.axes[0].plot(
                        x, D['mode wavelength']*np.ones(len(x)), '--', color='black')
                    self.figure_spectrogram.axes[0].plot(
                        x, D['res_wavelength'], color='gray', linewidth=3.0)
                delta_c = D['delta_c']
                delta_c_err = D['delta_c_error']
                delta_0 = D['delta_0']
                delta_0_err = D['delta_0_error']
                axes1[0].errorbar(x, delta_c, yerr=delta_c_err,
                                  label='{:.2f}'.format(D['mode wavelength']))
                axes1[1].errorbar(x, delta_0, yerr=delta_0_err,
                                  label='{:.2f}'.format(D['mode wavelength']))

                mode_intensity = (delta_c)/(delta_0+delta_c)**2
                threshold = (delta_0+delta_c)**3/delta_c

                axes2[0].plot(x, mode_intensity,
                              label='{:.2f}'.format(D['mode wavelength']))
                axes2[1].plot(x, threshold, label='{:.2f}'.format(
                    D['mode wavelength']))
            ...
            axes2[0].set_ylabel(
                r'$\frac {\delta_c} {(\delta_c+\delta_0)^{2}}$')
            axes2[1].set_ylabel(
                r'$\frac{(\delta_0+\delta_c)^{3}}{\delta_c}$, 1e12 $1/s^2$')
            axes1[0].legend()
            ...
            path, FileName = os.path.split(self.spectrogram_file_path)
            NewFileName = path+'\\' + \
                FileName.split('.')[-2]+'_mode_parameters.pkl'
            Data_to_save = {}
        ...
        if self.derive_taper_cavity_params:
            ...
            self.figure_spectrogram.axes[0].axvline(
                position_for_min_threshold, color='blue')
            ...
            for D in modes_parameters:
                ...
                length = np.nansum(
                    delta_c[1:]*np.diff(x_cutted))/np.nanmax(delta_c)
                plt.plot(x_cutted, Kerr_threshold(
                    D['res_wavelength'], delta_c*1e6, delta_0*1e6, length, self.SNAP.R_0), label='{:.2f}'.format(D['mode wavelength']))
    ...
    def plot_ERV_params(self, params_dict: dict, find_widths=True):
        ...
        non_res_transmission = non_res_transmission.reshape(
            len(positions), number_of_peaks_to_search)
        Fano_phase = Fano_phase.reshape(
            len(positions), number_of_peaks_to_search)
        ...
        self.S_print.emit(
            f'R_0 = {params_dict["R_0"]}, n = {params_dict["refractive_index"]}')
    ...
    def run_quantum_numbers_fitter(self):
        ...
        for i, wave in enumerate(resonances):
            ...
            y = y_min+(y_max-y_min)/fitter.th_resonances.pmax * \
                float(labels[i].split(',')[2])
            axes.annotate(labels[i], (wave, y))
        axes.set_title('R_fitted={} nm, T_best={} cost_function={}'.format(
            fitter.R_best, fitter.T_best, fitter.cost_best))
        self.single_spectrum_figure.canvas.draw()
